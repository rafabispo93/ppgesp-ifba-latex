VERSION = 1.0
TARBALL = ifbathesis-$(VERSION).tar.gz
UPLOAD_TO = app.dcc.ufba.br:~/public_html/ifbathesis/
...
%.dvi: %.tex ifbathesis.cls
	latex $<

%.pdf: %.tex ifbathesis.cls
	pdflatex $<
...
$(TARBALL): ifbathesis.cls abntex2-alf.bst
	tar czf $(TARBALL) $^
